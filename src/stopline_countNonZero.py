...
def detect_stopline(cal_image, low_threshold_value):
    ...
    #roi = edge_img[Offset : Offset+Gap, 0 : Width]
    ### 나중 조절
    stopline_roi = cal_image[Offset : Offset+Gap, lpos:rpos]
...
#set_roi(cal_image, 250, 350, 10)
    ...
    #검은색 x부분
    print(cv2.countNonZero(image), x_len * offset_y * 0.3)
    if cv2.countNonZero(image) > x_len * offset_y * 0.3 :
...
def image_processing(image, low_threshold_value):
    ...
    _, lane = cv2.threshold(L, low_threshold_value, 255, cv2.THRESH_BINARY_INV)
    #cv2.imshow("L", lane)
    return lane
...
while True:
    ...
    speed = detect_stopline(cal_image, 125)
    motor.speed = speed
    motor.angle = 0 #기본 유지
