...
def img_callback(data):
	...
	search_roi = black[0:50, 640-70:640]
        scale_percent = 200
        width = int(search_roi.shape[1]*scale_percent/100)
        height = int(search_roi.shape[0]*scale_percent/100)
        dim = (width, height)
        resized_roi = cv2.resize(search_roi, dim, cv2.INTER_AREA)
	num_whites = (cv2.countNonZero(search_roi))
	print(num_whites)
	cv2.imshow("search", search_roi)
        cv2.imshow("resized", resized_roi)
	cv2.waitKey(1)
